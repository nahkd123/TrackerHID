...
esp_err_t imu_setup() {
	...
	return ESP_OK;
	#elif IMU_PROTOCOL == IMU_PROTOCOL_SPI
	ESP_LOGI(TAG, "SPI protocol is not yet implemented for BMI160");
	return ESP_FAIL;
	#else
...
esp_err_t imu_poll(imu_data_t* data) {
	...
	bmi160_data_t bmi160_data = { .sensortime = 0 };
	TRY(last_error, i2c_master_write_read_device(IMU_I2C_PORT, IMU_BMI160_I2C_ADDRESS, &reg_data, 1, (uint8_t*)&bmi160_data, 15, -1));
